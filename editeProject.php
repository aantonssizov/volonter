...
$img_ids = R::getCol('SELECT (id) FROM images WHERE projects_id = ?', array($project->id));
...
if ( isset($_POST['submit']) ) {
  ...
  if ( isset($_FILES['logo_img']) ) {
    ...
    if (
        isset($project->ownImagesList[$img_ids[0]]) && 
        file_exists($folder . $project->ownImagesList[$img_ids[0]]->name)
      ) {
      unlink($folder . $project->ownImagesList[$img_ids[0]]->name);
      ...
      //-----------upload logo image-------------------
      $logo = R::load('images', $img_ids[0]);
      $logo->name = $logo_img;
      $logo->path = $folder;

      $project->ownImagesList[$img_ids[0]] = $logo;
    } else if ( isset($project->ownImagesList[$img_ids[0]]) && !empty($_FILES['logo_img']['name']) ) {
      $logo_img = $_FILES['logo_img']['name'];// Logo
  ...
  if ( isset($_FILES['img']) ) {
    ...
    foreach ($_FILES['img']['name'] as $key => $name) {
      if ( !empty($name) ) {
        if ( !empty($name) && isset($project->ownImagesList[$img_ids[$key + 1]]) && file_exists($folder.$project->ownImagesList[$img_ids[$key + 1]]->name) ) {
          unlink($folder.$project->ownImagesList[$img_ids[$key + 1]]->name);
        }
      }
    }
  ...
  header("Location: project.php?project_id=$project->id");
...
<html>
  <head>
    ...
    <title>Edite project -- volonter.ua</title>
  </head>
        ...
        <div class="container">
          ...
          <div class="row">
            <div class="col-lg-4 col-6">
              <input class="form-control m-1" type="text" name="title" value="<?php echo $project->title;?>" required placeholder="Title">
              <input class="form-control m-1" type="text" name="description" value="<?php echo $project->description;?>" placeholder="Description">
            </div>
            <div class="col-lg-4 col-6">
              <label class="btn btn-outline-secondary m-1 logo-img">
              Main image
                <input type="file" style="display: none;" name="logo_img">
                <span class="image_path"><?php echo @$project->ownImagesList[$img_ids[0]]->name;?></span>
              </label>
            </div>
          </div>
          <div class="row"> 
            <div class="col-lg-8 col-12">
              <textarea class="form-control m-1" placeholder="Text" rows="5" cols="3" name="text"><?php echo $project->text;?></textarea>
            </div>
            <div class="col-lg-4 col-12">
              ...
              <input class="form-control m-1" placeholder="Address" name='address' value="<?php echo $project->address?>">
            </div>
          </div>
          <div class="row"> 
            <div class="col-lg-8 col-12">
              <div class="row">
                <label class="btn btn-outline-secondary m-1 col _img_">Add image
                  ...
                  <span class="image_path"><?php echo @$project->ownImagesList[$img_ids[1]]->name;?></span>
                </label>
                <label class="btn btn-outline-secondary m-1 col _img_">Add image
                  <input type="file" style="display: none;" name="img[1]">
                  <span class="image_path"><?php echo @$project->ownImagesList[$img_ids[2]]->name;?></span>
                </label>
                <label class="btn btn-outline-secondary m-1 col _img_">Add image
                  <input type="file" style="display: none;" name="img[2]">
                  <span class="image_path"><?php echo @$project->ownImagesList[$img_ids[3]]->name;?></span>
                </label>
              </div>
            </div>
            <div class="col-lg-4 col-12">
              <button class="btn btn-success btn-block btn-lg m-1" type="submit" name="submit">Update project</button>
            </div>
          </div>
        </div>
      </div>
